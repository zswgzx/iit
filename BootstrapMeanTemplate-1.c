...
#define HDR 352
typedef unsigned short sub_mask;
...
{ 
  ...
  sub_mask *countsub,*countmean,zeroshort=0;
  ...
  countmean = malloc(sizeof(sub_mask)*SSIZE);
  if (countmean == NULL) {
  ...
  for(i=0;i< COMBI;i++) {
	...
	for(r=1;r<=YRES;r++)
	  for(c=1;c<=XRES;c++) {
	    fwrite(&zeroshort,sizeof(sub_mask),1,fp_count);
	  }
	...
	if((fread(countmean,sizeof(sub_mask),SSIZE,fp_count))!=SSIZE) {
	...
	if ((fwrite(countmean,sizeof(sub_mask),SSIZE,fp_count))!=SSIZE) {
